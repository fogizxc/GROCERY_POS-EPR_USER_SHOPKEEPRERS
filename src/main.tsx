...
function redirectToSuperAdmin() {
  if (!window.location.pathname.startsWith('/super-admin')) window.history.replaceState({}, '', '/super-admin');
}
...
function AppGate() {
  ...
  if (!authenticated) return <AuthScreen onAuthenticated={() => { const role = readRoleFromToken(); if (role === 'super_admin') { localStorage.setItem('freshcart_role', role); redirectToSuperAdmin(); window.location.reload(); return; } if (isSuperPortal) { localStorage.removeItem('freshcart_token'); localStorage.removeItem('freshcart_role'); window.location.reload(); return; } localStorage.setItem('freshcart_role', role); setAuthenticated(true); }} />;

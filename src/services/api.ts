...
export interface ApiPayment { id: string; orderId: string; method: PaymentMethod; status: string; amount: number; provider?: string; providerPaymentId?: string; createdAt: string; }
export interface ApiOrder { id: string; customerId: string; shopId: string; items: ApiOrderItem[]; subtotal: number; deliveryFee: number; total: number; paymentMethod: PaymentMethod; status: OrderStatus; createdAt: string; deliverySlotId?: string; }
export interface ApiDeliveryQueueItem extends ApiOrder { deliveryStatus?: DeliveryStatus; deliveryEmployeeId?: string; }
...
export const api={
 ...
 orders:(params?:{status?:OrderStatus;shopId?:string})=>request<ApiOrder[]>(`/orders${query(params)}`), customerOrders:()=>request<ApiOrder[]>('/customer/orders'), customerOrder:(id:string)=>request<ApiOrderDetail>(`/customer/orders/${id}`), createOrder:(input:{shopId:string;items:{productId:string;quantity:number}[];paymentMethod:PaymentMethod;addressId:string;deliverySlotId:string;idempotencyKey?:string})=>request<ApiOrder&{address:ApiAddress;deliverySlot:ApiDeliverySlot;payment:ApiPayment}>('/orders',{method:'POST',headers:{'Idempotency-Key':input.idempotencyKey??createIdempotencyKey()},body:JSON.stringify(input)}),
 createPaymentOrder:(orderId:string)=>request<RazorpayCheckoutOrder>('/payments/create-order',{method:'POST',body:JSON.stringify({orderId})}),
 verifyPayment:(input:{orderId:string;razorpayOrderId:string;razorpayPaymentId:string;razorpaySignature:string})=>request<{ok:boolean;orderId:string;paymentId:string;status:string}>('/payments/verify',{method:'POST',body:JSON.stringify(input)}),
 reorder:(orderId:string)=>request<ApiReorder>(`/customer/orders/${orderId}/reorder`,{method:'POST'}), cancelOrder:(id:string)=>request<ApiOrder>(`/orders/${id}/status`,{method:'PATCH',body:JSON.stringify({status:'CANCELLED'})}),

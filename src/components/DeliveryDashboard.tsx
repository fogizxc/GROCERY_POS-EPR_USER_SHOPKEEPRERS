...
import { CheckCircle2, MapPin, PackageCheck, RefreshCw, Truck, UserRound } from 'lucide-react';
import { api, type ApiOrder, type ApiUser, type DeliveryStatus } from '../services/api';
...
export function DeliveryDashboard({ admin = false, flash }: Props) {
  ...
  const advance = async (order: ApiOrder) => {
    const status = (next[order.status as DeliveryStatus] ?? null);
    if (!status) return;
    setBusy(order.id);
    try { await api.deliveryStatus(order.id, status); await load(); flash(`${order.id} → ${status.replaceAll('_', ' ')}`); }
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8">
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-3"><Kpi icon={<Truck/>} label="Queue" value={String(orders.length)} sub="delivery orders"/><Kpi icon={<PackageCheck/>} label="In transit" value={String(orders.filter(o => o.status === 'OUT_FOR_DELIVERY').length)} sub="out for delivery"/><Kpi icon={<CheckCircle2/>} label="Delivered" value={String(orders.filter(o => o.status === 'DELIVERED').length)} sub="completed"/></section>
    <section className="mt-7 rounded-3xl bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center gap-2"><MapPin className="text-[#5d836b]" size={20}/><h2 className="heading text-xl font-extrabold text-[#173d2e]">Delivery queue</h2></div><div className="mt-4 space-y-3">{orders.map(order => { const current = steps.indexOf(order.status as DeliveryStatus); const target = next[order.status as DeliveryStatus]; return <div key={order.id} className="rounded-2xl border border-black/5 bg-[#fafbf8] p-4"><div className="flex flex-wrap items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#eaf1ea] text-[#3c7358]"><PackageCheck size={18}/></div><div className="min-w-[190px] flex-1"><div className="text-sm font-extrabold text-[#203229]">{order.id}</div><div className="mt-1 text-xs text-[#7d8b83]">{order.items.reduce((s, i) => s + i.quantity, 0)} items • ₹{order.total}</div></div><span className="rounded-full bg-[#edf3ee] px-3 py-1 text-[10px] font-extrabold uppercase tracking-wider text-[#47715a]">{order.status.replaceAll('_', ' ')}</span>{target && <button disabled={busy === order.id} onClick={() => void advance(order)} className="rounded-xl bg-[#173d2e] px-3 py-2 text-xs font-extrabold text-white disabled:opacity-50">{busy === order.id ? 'Updating…' : target === 'DELIVERED' ? 'Mark delivered' : target.replaceAll('_', ' ')}</button>}</div><div className="mt-4 flex gap-1">{steps.map((step, index) => <div key={step} className={`h-1.5 flex-1 rounded-full ${index <= current ? 'bg-[#5d836b]' : 'bg-[#e6ebe6]'}`}/>)}</div>{admin && <div className="mt-4 flex flex-wrap gap-2"><select value={selectedEmployee[order.id] ?? ''} onChange={e => setSelectedEmployee(v => ({ ...v, [order.id]: e.target.value }))} className="min-w-[220px] rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold"><option value="">Assign employee…</option>{employees.map(employee => <option key={employee.id} value={employee.id}>{employee.name} • {employee.phone}</option>)}</select><button disabled={busy === order.id} onClick={() => void assign(order.id)} className="rounded-xl bg-[#eaf1ea] px-4 py-2 text-xs font-extrabold text-[#315245]">Assign delivery</button></div>}</div>; })}{!orders.length && <div className="py-12 text-center text-sm text-[#7d8c83]">No delivery orders in the queue.</div>}</div></section>
  </main>;
}

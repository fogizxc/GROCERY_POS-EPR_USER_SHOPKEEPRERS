...
type LoginRole = Extract<Role, 'customer' | 'shopkeeper' | 'employee' | 'admin'>;
...
export function AuthScreen({ onAuthenticated }: AuthScreenProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        await api.login(identifier.trim(), role, password);
      } else {
  ...
  const switchMode = (next: 'login' | 'register') => {
    setMode(next);
    setError('');
    if (next === 'register') setRole('customer');
  };
    ...
    <main className="min-h-screen bg-[#f7f7f2] px-4 py-8 sm:px-6">
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-5xl overflow-hidden rounded-[32px] bg-white shadow-xl lg:grid-cols-2">
        <section className="hidden bg-[#173d2e] p-10 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <p className="text-xs text-[#b9d9bf]">Secure accounts • Role-based access • FreshCart ERP</p>
        </section>
        ...
        <section className="p-6 sm:p-10 lg:p-12">
          <div className="mx-auto max-w-md">
            ...
            <div className="mt-10 lg:mt-4"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#819087]">Secure workspace</p><h2 className="heading mt-2 text-3xl font-extrabold text-[#173d2e]">{mode === 'login' ? 'Welcome back' : 'Create your account'}</h2><p className="mt-2 text-sm text-[#74837b]">{mode === 'login' ? 'Sign in to your FreshCart workspace.' : 'Create a customer account and start your first grocery order.'}</p></div>

            {mode === 'login' && <div className="mt-7 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {(Object.keys(roleLabels) as LoginRole[]).map(item => <button key={item} type="button" onClick={() => { setRole(item); setError(''); }} className={`rounded-xl px-2 py-2.5 text-[11px] font-extrabold transition ${role === item ? 'bg-[#173d2e] text-white' : 'bg-[#f3f5ef] text-[#52655b] hover:bg-[#e9eee6]'}`}>{roleLabels[item]}</button>)}
            </div>}
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
              ...
              {mode === 'register' && <p className="rounded-2xl bg-[#f3f7ef] px-4 py-3 text-[11px] leading-5 text-[#5f7067]">Passwords must be at least 8 characters and include both letters and numbers.</p>}
              {error && <div role="alert" className="rounded-2xl bg-[#fff0ed] px-4 py-3 text-xs font-semibold text-[#a14335]">{error}</div>}
              <button disabled={saving} className="flex w-full items-center justify-center rounded-2xl bg-[#173d2e] px-5 py-3.5 text-sm font-extrabold text-white transition hover:bg-[#24523e] disabled:opacity-50">{saving?'Please wait…':mode==='login'?`Sign in as ${roleLabels[role]}`:'Create customer account'}{!saving&&<ArrowRight size={16} className="ml-2"/>}</button>
            </form>

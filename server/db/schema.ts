...
export async function ensureIndexes(client: MongoClient) {
  ...
  await Promise.all([
    ...
    db.collection('products').createIndex({ barcode: 1 }, { unique: true, sparse: true }),
    ...
    db.collection('payments').createIndex({ providerOrderId: 1 }, { unique: true, sparse: true }),
    db.collection('paymentWebhookEvents').createIndex({ eventId: 1 }, { unique: true }),
    db.collection('paymentWebhookEvents').createIndex({ receivedAt: -1 }),
    db.collection('deliveryAssignments').createIndex({ orderId: 1 }, { unique: true }),
    db.collection('deliveryAssignments').createIndex({ employeeId: 1, status: 1 }),
    ...
    db.collection('auditLogs').createIndex({ entity: 1, entityId: 1, createdAt: -1 }),
  ]);
}

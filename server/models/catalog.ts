...
export interface Payment { id: string; orderId: string; method: 'UPI' | 'CARD' | 'COD'; status: 'PENDING' | 'AUTHORIZED' | 'PAID' | 'FAILED' | 'REFUND_PENDING' | 'REFUNDED' | 'CANCELLED'; amount: number; provider?: string; providerPaymentId?: string; createdAt: string; }
export interface DeliveryAssignment { id: string; orderId: string; shopId: string; employeeId?: string; status: 'UNASSIGNED' | 'ASSIGNED' | 'PICKED_UP' | 'OUT_FOR_DELIVERY' | 'DELIVERED' | 'FAILED'; assignedAt?: string; deliveredAt?: string; }

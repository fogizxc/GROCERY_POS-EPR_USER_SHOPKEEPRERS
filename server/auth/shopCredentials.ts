...
export async function listPartnerCredentialPool(kind:CredentialKind) { const db=mongoDb(); if(!db)return []; const rows=await db.collection<PartnerCredentialEntry>('partnerCredentialPool').find({kind}).sort({slot:1}).toArray(); return rows.map(row=>({slot:row.slot,baseLoginId:row.baseLoginId,assignedToId:row.assignedToId,assignedAt:row.assignedAt,active:row.active})); }

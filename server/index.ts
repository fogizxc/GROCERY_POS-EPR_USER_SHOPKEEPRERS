...
app.use('/api',rateLimit({windowMs:60*1000,max:240})); app.use('/api/auth',rateLimit({windowMs:15*60*1000,max:30}),auth); app.use('/api/onboarding',rateLimit({windowMs:15*60*1000,max:12}),onboarding); app.use('/api/shopkeeper-portal',rateLimit({windowMs:60*1000,max:120}),shopkeeperPortal);
app.use('/api',inventory); app.use('/api',orderCancellation); app.use('/api/features/rewards',rewards); app.use('/api',api); app.use('/api/bootstrap',bootstrap); app.use('/api/customer',customer); app.use('/api/admin',admin); app.use('/api/shopkeeper',shopkeeper); app.use('/api/delivery',delivery); app.use('/api/ops',ops); app.use('/api/payments',paymentsRouter); app.use('/api/pickup',pickup); app.use('/api/features',features);
const frontendDist=path.resolve(process.cwd(),'dist'); app.use(express.static(frontendDist,{index:'index.html',maxAge:isProduction?'1d':0})); app.get(/^(?!\/api(?:\/|$)).*/,(_req,res,next)=>res.sendFile(path.join(frontendDist,'index.html'),error=>error&&next(error))); app.use((_req,res)=>res.status(404).json({error:'Route not found'})); app.use((error:unknown,_req,res,_next:express.NextFunction)=>{console.error(error);res.status(500).json({error:error instanceof Error?error.message:'Internal server error'})});

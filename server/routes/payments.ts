...
paymentsRouter.post('/create-order', requireAuth, async (req, res) => {
  ...
  if (mongoDb()) {
    ...
    try {
      ...
      return res.status(502).json({ error: error instanceof Error ? error.message : 'Unable to create payment order' });
    }
  }
...
paymentWebhook.post('/', async (req, res) => {
  ...
  let payload: { event?: string; payload?: { payment?: { entity?: { id?: string; order_id?: string; status?: string } }; order?: { entity?: { id?: string; status?: string } } } };
  ...
  if (mongoDb() && razorpayOrderId) {
    ...
    if (event === 'payment.captured' || event === 'order.paid') {
      await db.collection<Payment>('payments').updateOne({ providerPaymentId: razorpayOrderId }, { $set: { status: 'PAID', provider: 'razorpay' } });
    } else if (event === 'payment.failed') {
      await db.collection<Payment>('payments').updateOne({ providerPaymentId: razorpayOrderId }, { $set: { status: 'FAILED', provider: 'razorpay' } });
    }
  }

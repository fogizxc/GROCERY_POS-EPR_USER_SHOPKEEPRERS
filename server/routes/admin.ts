...
admin.get('/dashboard', async (_req, res) => {
  try {
    ...
    console.error('Admin dashboard MongoDB read failed:', error);
  }
...
admin.get('/super-dashboard', requireSuperAdmin, async (_req, res) => {
  try {
    ...
    console.error('Super dashboard MongoDB read failed:', error);
  }
...
admin.post('/onboarding/applications/:referenceId/assign-credential', async (req,res) => {
  ...
  if(!/^\d{4}$/.test(rawSuffix))return res.status(400).json({error:'Enter exactly four digits to append to the 18-digit credential'});
  const slot=Number(rawSuffix);
  if(slot<1||slot>100)return res.status(400).json({error:'For the pre-built pool, choose a suffix from 0001 to 0100'});
  ...
  const base=await getPartnerCredentialBase(kind,slot);
  if(!base)return res.status(404).json({error:'Credential slot not found'});
  if(base.assignedToId)return res.status(409).json({error:'That four-digit suffix is already assigned'});
  const loginId=`${base.baseLoginId}${rawSuffix}`;
  const password=`${base.basePassword}${rawSuffix}`;
  if(!/^\d{22}$/.test(loginId)||!/^\d{22}$/.test(password))return res.status(500).json({error:'Credential generator produced an invalid length'});
  ...
  if(!claimed)return res.status(409).json({error:'That four-digit suffix was just assigned. Choose another suffix.'});
